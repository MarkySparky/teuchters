...
deepMatches(Mark, { name: 'Mark' }); // true
deepMatches(Mark, { age: 21 }); // false
deepMatches(Mark, { pets: [ { type: 'cat' } ] }); // true
deepMatches(Mark, { pets: [ { name: 'Hawk' } ] }); // true
deepMatches(Mark, { pets: [ { name: 'Hairball' } ] }); // false
```
...
pick(user, keys); // {firstName:"Mark", dob: "1985/07/23"}

// or multiple arguments
pick(user, 'firstName', 'lastName'); // {firstName:"Mark", lastName: "Doe"}
```
...
pluck(users, 'name'); // {first: 'Mark', second: 'Mary'} );
